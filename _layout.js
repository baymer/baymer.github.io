...
;
/* ../../desktop.blocks/layout/layout.js begin */
modules.define('i-bem__dom', ['jquery'], function(provide, $, BEMDOM) {
    ...
    BEMDOM.decl('layout', {
        beforeSetMod: {
            js: {
                ...
                // console.time('test');
                    var self = this,
                        totalHeight = 0,
                        heightMainCel = self.findElem('cel', 'main', 'yes').height() || 0,
                        amountVisibleCols = parseInt(self.getMod('columns'), 10),
                        cels = self.findElem('cel', 'main', 'no'),
                        celsContainer = [];


                    cels.each(function(i, cel) {
                        var height = cel.offsetHeight;

                        cel.innerText = "position: " + i + "; " + cel.innerHTML;
                        totalHeight += height;
                    ...
                    celsContainer = smallSort(celsContainer);

                    function smallSort(arr, numChange) {
                        var sorted = arr.slice(numChange).sort(function(a, b) {
                            return b.height - a.height;
                        });

                        return arr.slice(0, arr.length + numChange).concat(sorted);
                    }

                    averageHeight = (( totalHeight + heightMainCel ) / amountVisibleCols ^ 0);

                    self._cols = self.findElem('col', 'size', 'normal').map(function(i, item) { return $(item); });
                    self._averageHeight = averageHeight;
                    self._celsContainer = celsContainer;
                    self._heightMainCel = heightMainCel;
                    self._cels = cels;

                    self._resizeLayout();
                    BEMDOM.win.on('resize', self._resizeLayout.bind(self));
                }
            }
        },
        ...
        onSetMod: {
            columns: {
                2: function() { this._sortCels(2); },
                3: function() { this._sortCels(3); },
                4: function() { this._sortCels(4); }
            }
        },

        _resizeLayout: function() {
            var self = this,
                width = self.domElem.width(),
                amountVisibleCols = Math.floor(width / 390);

            if (amountVisibleCols < 2) {
                amountVisibleCols = 2;
            } else if (amountVisibleCols > 4) {
                amountVisibleCols = 4;
            }

            self.setMod('columns', amountVisibleCols);
        },

        _sortCels: function(amountVisibleCols) {
        // console.time('test');
            var self = this,
                $cols = self._cols,
                cels = self._cels,
                ...
                celsContainer = self._celsContainer,

                heightMainCel = self._heightMainCel,
                averageHeight = self._averageHeight,
                i = 0;

            $cols.each(function(i, $col) {
                var height = 0,
                    alreadySortedCells = $col._alreadySortedCells && $col._alreadySortedCells[amountVisibleCols];

                if (alreadySortedCells) {
                    BEMDOM.append($col, alreadySortedCells);
                    isAlreadySorted = true;
                    return;
                } else if (!$col._alreadySortedCells) {
                    $col._alreadySortedCells = {};
                }

                if (i < 2) {
                    height = heightMainCel;
                }
            ...
            celsContainer.forEach(function(item) {
                var celHeight = item.height,
                    minColHeight = 10e8,
                    $col = null;


                $cols.each(function(i, $cCol) {
                    if (i < amountVisibleCols && $cCol._height < minColHeight) {
                        $col = $cCol;
                        minColHeight = $cCol._height;
                    }
                });

                $col._height += celHeight;
                $col._container.push(item);

            });

            $cols.each(function(i, $col) {
                var container = $col._alreadySortedCells[amountVisibleCols] = $col._container.sort(function(a, b) {
                    return a.pos - b.pos;
                }).map(function(item) {return item.cel;});

                BEMDOM.append($col,  container);
            });
                    // console.timeEnd('test');
        }
    });
...
/* ../../desktop.blocks/layout/layout.js end */
;
